...
def expand_patch_embedding_for_validation(transformer):
    ...
    # Exact structural equivalence: the old channel prefix is untouched and
    # only the four new trailing channels are zero.
    if not torch.equal(
        new_patch_embedding.weight[:, :old_in_channels],
        old_patch_embedding.weight,
    ):
        raise RuntimeError("Pretrained Patchify channel weights changed.")
    if torch.count_nonzero(
        new_patch_embedding.weight[:, old_in_channels:]
    ).item() != 0:
        raise RuntimeError("Validation mask channels were not zero initialized.")
    if old_patch_embedding.bias is not None and not torch.equal(
        new_patch_embedding.bias, old_patch_embedding.bias
    ):
        raise RuntimeError("Pretrained Patchify bias changed.")
